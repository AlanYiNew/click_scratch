...
Camkes_IPRouteTable defines an interface useful for implementing IPv4 route lookup
...
Click provides several elements that implement all or part of the Camkes_IPRouteTable
...
These four Camkes_IPRouteTable virtual functions should generally be overridden by
...
The following functions, overridden by Camkes_IPRouteTable, are available for use by
subclasses.
...
class Camkes_IPRouteTable : public Element { public:

    void* cast(const char*);
    int configure(Vector<String>&, ErrorHandler*) CLICK_COLD;
    void add_handlers() CLICK_COLD;

    Camkes_IPROuteTable() = default;
